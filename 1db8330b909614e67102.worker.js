...
// BASIC UI
//
const toggleSiteInfoOpen=()=>dispatch=>{Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'open-site-info'});return{type:TOGGLE_SITE_INFO_OPEN};};const toggleAlgInfoOpen=()=>(dispatch,getState)=>{const algorithm=getState();Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'open-algorithm-info',algorithm});return{type:TOGGLE_ALG_INFO_OPEN};};//
// MAP INTERACTION
//
const setViewportState=viewport=>({type:SET_VIEWPORT_STATE,viewport});//
// SOLVER CONTROLS
//
const resetEvaluatingStateAction=()=>({type:RESET_EVALUATING_STATE});const resetBestPathStateAction=()=>({type:RESET_BEST_PATH_STATE});const setAlgorithmAction=(algorithm,defaults)=>({type:SET_ALGORITHM,algorithm,defaults});const startSolvingAction=(points,delay,evaluatingDetailLevel)=>({type:START_SOLVING,points,delay,evaluatingDetailLevel});const stopSolvingAction=()=>({type:STOP_SOLVING});const setAlgorithm=(algorithm,defaults={})=>dispatch=>{Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'select-algorithm',algorithm});dispatch(resetEvaluatingStateAction());dispatch(setAlgorithmAction(algorithm,defaults));};const setDelay=delay=>dispatch=>{Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'set-delay',delay});return{type:SET_DELAY,delay};};const setEvaluatingDetailLevel=level=>({type:SET_EVALUATING_DETAIL_LEVEL,level});const setShowBestPath=show=>({type:SET_SHOW_BEST_PATH,show});const resetSolverState=()=>dispatch=>{Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'reset-solver'});dispatch(resetEvaluatingStateAction());dispatch(resetBestPathStateAction());};const startSolving=(...args)=>(dispatch,getState)=>{const{algorithm}=getState();Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'start-solving',algorithm});dispatch(resetEvaluatingStateAction());dispatch(startSolvingAction.apply(void 0,args));};const stopSolving=()=>dispatch=>{Object(_emitCustomEvent__WEBPACK_IMPORTED_MODULE_0__["default"])({event:'stop-solving'});dispatch(resetEvaluatingStateAction());dispatch(stopSolvingAction());};//
// SOLVER ACTIONS
//
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = (ev=>{if(typeof window!=="undefined"&&window.dataLayer){console.log(ev);window.dataLayer.push(ev);}});

/***/ })

/******/ });
//# sourceMappingURL=1db8330b909614e67102.worker.js.map