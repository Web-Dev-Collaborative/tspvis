...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/solvers/heuristic/arbitraryInsertion.worker.js");
/******/ })
/************************************************************************/
/******/ ({
...
/***/ "./src/solvers/heuristic/arbitraryInsertion.worker.js":
/*!************************************************************!*\
  !*** ./src/solvers/heuristic/arbitraryInsertion.worker.js ***!
  \************************************************************/
/*! no exports provided */
...
const arbitraryInsertion = async points => {
  ...
  //
  points.sort((a, b) => (
    Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[0], b) -
    Object(_cost__WEBPACK_IMPORTED_MODULE_1__["distance"])(path[0], a)
  ));
  ...
  while (points.length > 0) {
    ...
    for (let i=1; i<path.length; i++) {
      const insertionCost = Object(_cost__WEBPACK_IMPORTED_MODULE_1__["pathCost"])([
...
/* eslint-disable no-restricted-globals */const wrapSolver=solver=>async(...args)=>{await solver.apply(void 0,args);self.postMessage(_store_actions__WEBPACK_IMPORTED_MODULE_0__["stopSolvingAction"]());self.terminate();};const makeSolver=solver=>{const run=wrapSolver(solver);self.solverConfig={detailLevel:0,delay:10,fullSpeed:false};self.setBestPath=(...args)=>{self.postMessage(_store_actions__WEBPACK_IMPORTED_MODULE_0__["setBestPath"].apply(_store_actions__WEBPACK_IMPORTED_MODULE_0__,args));};self.setEvaluatingPaths=(getPaths,level=1)=>{if(self.solverConfig.detailLevel>=level){const{paths,cost}=getPaths();self.postMessage(_store_actions__WEBPACK_IMPORTED_MODULE_0__["setEvaluatingPaths"](paths,cost));}};self.setEvaluatingPath=(getPath,level=1)=>{if(self.solverConfig.detailLevel>=level){const{path,cost}=getPath();self.postMessage(_store_actions__WEBPACK_IMPORTED_MODULE_0__["setEvaluatingPath"](path,cost));}};self.sleep=()=>{const duration=self.solverConfig.fullSpeed?0:self.solverConfig.delay||10;return new Promise(resolve=>{setTimeout(resolve,duration);});};self.onmessage=async({data:action})=>{switch(action.type){case _store_actions__WEBPACK_IMPORTED_MODULE_0__["START_SOLVING"]:self.solverConfig.delay=action.delay;self.solverConfig.detailLevel=action.evaluatingDetailLevel;self.solverConfig.fullSpeed=action.fullSpeed;run(action.points);break;case _store_actions__WEBPACK_IMPORTED_MODULE_0__["SET_DELAY"]:self.solverConfig.delay=action.delay;break;case _store_actions__WEBPACK_IMPORTED_MODULE_0__["SET_EVALUATING_DETAIL_LEVEL"]:self.solverConfig.detailLevel=action.level;break;case _store_actions__WEBPACK_IMPORTED_MODULE_0__["GO_FULL_SPEED"]:self.solverConfig.fullSpeed=true;break;default:throw new Error(`invalid action sent to solver ${action.type}`);}};};/* harmony default export */ __webpack_exports__["default"] = (makeSolver);
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "goFullSpeedAction", function() { return goFullSpeedAction; });
...
/******/ });
//# sourceMappingURL=b70c9ffe3f4a1ad18b90.worker.js.map